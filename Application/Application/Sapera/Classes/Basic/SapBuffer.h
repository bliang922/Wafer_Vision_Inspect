...
public:
   ...
   /* Ensure the buffer is created and its parameters have the requested values
    *
    * If the buffer has been already created with the same parameter values,
    * this method does not re-create it and does not modify it in any way.
    *
    * If the buffer has been already created with different parameter values,
    * this method will destroy the buffer and re-create it with requested parameters.
    *
    * This method is expected to fail if the buffer is associated with a SapTransfer object.
    */
   BOOL RecreateWithParameters(int new_count, int new_width, int new_height, SapFormat new_format);
   ...
   virtual BOOL GetHostPerformanceCounterStamp(UINT64 *pCounterStamp);
   virtual BOOL GetHostPerformanceCounterStamp(int index, UINT64 *pCounterStamp);
